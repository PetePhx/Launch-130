...
# we use duplicate array so as to avoid mutating the original array
...
def drop_while(arr)
  arr.reduce(arr.dup) { |sum, i| yield(i) ? sum = sum.drop(1) : (return sum) }
end
...
p drop_while([1, 3, 5, 6]) { |value| value < 5 } == [5, 6]
p drop_while([]) { |value| true } == []
p drop_while([1, 6, 1]) { |value| value.odd? } == [6, 1]

...
# we have it return an array of the block return values, similar to #map

def step(a, b, s, &block)
  out = []
  while (num ||= a) <= b
    out << (block ? block.call(num) : num)
    num += s
  end
  out
end
...
# => [nil, nil, nil, nil]
# since #puts returns nil

p step(1, 10, 3)
# => [1, 4, 7, 10]
p step(1, 10, 3).select(&:even?)
# [4, 10]
p step(1, 10, 3).each
# #<Enumerator: [1, 4, 7, 10]:each>
